...
export const BooleanPropertyTest = ({ src }: { src: string }) => {
  ...
  const { value: agreedToTerms, setValue: setAgreedToTerms } =
    useViewModelInstanceBoolean('agreedToTerms', rive?.viewModelInstance);
...
export const ColorPropertyTest = ({ src }: { src: string }) => {
  ...
  const { value: colorNum, setRgb } = useViewModelInstanceColor(
    'favColor',
    rive?.viewModelInstance
  );
        ...
        <div>
          <label>
            ...
            <div
              ...
              style={{
                backgroundColor:
                  typeof colorNum === 'string'
                    ? colorNum
                    : colorNumberToHexString(colorNum),
                width: '20px',
                height: '20px',
...
export const EnumPropertyTest = ({ src }: { src: string }) => {
  ...
  const {
    ...
    setValue: setCountry,
    values: countries,
  } = useViewModelInstanceEnum('country', rive?.viewModelInstance);
...
export const NestedViewModelTest = ({ src }: { src: string }) => {
  ...
  const {
    ...
    values: drinkTypes,
  } = useViewModelInstanceEnum('favDrink/type', rive?.viewModelInstance);
...
export const TriggerPropertyTest = ({ src }: { src: string }) => {
  ...
  const { trigger: onFormSubmit } = useViewModelInstanceTrigger(
    'onFormSubmit',
    rive?.viewModelInstance,
    {
      onTrigger: () => {
  ...
  const { trigger: onFormReset } = useViewModelInstanceTrigger(
    'onFormReset',
    rive?.viewModelInstance,
    {
      onTrigger: () => {
...
// Component to demonstrate different view model instances
export const PersonInstances = ({ src }: { src: string }) => {
  ...
  const params = useDefaultInstance
    ? { useDefault: true, rive }
    : { name: activeInstance, rive };
  ...
  const { value: country } = useViewModelInstanceEnum(
    'country',
    viewModelInstance
  );
    ...
    >
      ...
      {currentImageUrl && (
        <div style={{ fontSize: '12px', color: '#666' }}>
          <span data-testid="current-image-url">
...
export const FontPropertyTest = ({ src }: { src: string }) => {
  ...
  const { setValue: setSubheadingFont } = useViewModelInstanceFont(
    'subheadingFont',
    rive?.viewModelInstance
  );
...
const TodoItemComponent = ({
  ...
  const { value: text, setValue: setText } = useViewModelInstanceString(
    'text',
    todoItem
  );
  const { value: isDone, setValue: setIsDone } = useViewModelInstanceBoolean(
    'isDone',
    todoItem
  );
  ...
  return (
    ...
    >
      ...
      />
      <div
        data-testid={`todo-text-value-${index}`}
        style={{ fontSize: '12px', color: '#666' }}
      >
        Text: {text}
      </div>
      <div
        data-testid={`todo-done-value-${index}`}
        style={{ fontSize: '12px', color: '#666' }}
      >
      ...
      {rive === null ? (
        ...
        <div>
          <div style={{ marginBottom: '20px' }}>
            ...
            <div data-testid="artboard1-current">
          ...
          <div>
            ...
            <div data-testid="artboard2-current">
