...
import {
  ...
  FontPropertyTest,
  TodoListTest,
  ArtboardPropertyTest,
} from './DataBindingTests';
...
export const BooleanPropertyStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const termsCheckbox =
      canvas.getByTestId<HTMLInputElement>('terms-checkbox');
...
export const EnumPropertyStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      const countrySelect =
        canvas.getByTestId<HTMLSelectElement>('country-select');
    ...
    const optionValues = Array.from(countrySelect.options).map(
      (option) => option.value
    );
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('country-value').textContent).toBe(
        optionToSelect
      );
    });
  },
...
export const NestedViewModelStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(
      () => {
        const drinkTypeSelect =
          canvas.getByTestId<HTMLSelectElement>('drink-type-select');
    ...
    const drinkTypeSelect =
      canvas.getByTestId<HTMLSelectElement>('drink-type-select');
    const optionValues = Array.from(drinkTypeSelect.options).map(
      (option) => option.value
    );
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('drink-type-value').textContent).toBe(
        optionToSelect
      );
    });
  },
...
export const TriggerPropertyStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(canvas.getByTestId('callback-triggered').textContent).toContain(
      'none'
    );
...
export const PersonInstancesStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('instance-name').textContent).toContain(
        'Default'
      );
    });
...
export const ImagePropertyStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(imageUrlElement.textContent).toContain(
      'Current image: https://picsum.photos'
    );
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(
      () => {
        ...
        expect(canvas.getByTestId('set-font-orbitron')).toBeTruthy();
    ...
    await userEvent.click(canvas.getByTestId('set-font-orbitron'));

    await waitFor(
      () => {
        expect(canvas.getByTestId('current-font').textContent).toBe(
          'Current font: Orbitron'
        );
      },
      { timeout: 5000 }
    );
...
export const TodoListStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const initialCount = parseInt(
      initialLengthText?.match(/Items: (\d+)/)?.[1] || '0'
    );
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('list-length').textContent).toContain(
        `Items: ${initialCount + 1}`
      );
    });
    ...
    if (currentCount > 0) {
      ...
      await waitFor(
        () => {
          expect(canvas.getByTestId('todo-text-value-0').textContent).toContain(
            'Test Item'
          );
        },
        { timeout: 3000 }
    ...
    if (currentCount >= 2) {
      const firstText =
        canvas.getByTestId<HTMLInputElement>('todo-text-0').value;
      const secondText =
        canvas.getByTestId<HTMLInputElement>('todo-text-1').value;
...
export const ArtboardPropertyStory: StoryObj = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(canvas.getByTestId('artboard1-current').textContent).toBe(
      'Current: None'
    );
    expect(canvas.getByTestId('artboard2-current').textContent).toBe(
      'Current: None'
    );
